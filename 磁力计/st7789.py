...
class ST7789():
    ...
    def rotation(self, rotation):
        ...
        if self._rotation == 0:         # Portrait
            ...
            self.width = self._display_width
            self.height = self._display_height+80
            ...
            self.width = self._display_height+80
            self.height = self._display_width
            if self._display_width == 135:
                self.xstart = 40
                self.ystart = 52
    ...
    def set_window( self, x, y, w, h ):
        ...
        self.buf4[2] = x1 >> 8
        self.buf4[3] = x1 &  0xff
        self.write( ST7789_CASET, self.buf4 )
        self.buf4[0] = y0 >> 8
        self.buf4[1] = y0 &  0xff
        self.buf4[2] = y1 >> 8
        self.buf4[3] = y1 &  0xff
        self.write( ST7789_RASET, self.buf4 )
    ...
    def clear_area( self, color, x, y, w, h):
        self.buf2[0] = color >> 8
        self.buf2[1] = color &  0xff
...
'''
    ...
    def write_dma( self, command=None, data=None ):
        if self.cs:
            self.cs.off()
        if command is not None:
            self.dc.off()
    ...
    def draw_bitmap_dma( self, x, y, w, h, data, is_blocking=True ):
        self.set_window( x, y, w, h )
        self.write_dma( ST7789_RAMWR, data, is_blocking )
'''   
'''
    def _set_columns(self, start, end):
        ...
        if start <= end <= self.width:
            self.write_dma(ST7789_CASET, _encode_pos(
    ...
    def _set_rows(self, start, end):
       ...
       """
        if start <= end <= self.height:
            self.write_dma(ST7789_RASET, _encode_pos(
...
'''            
    ...
    def pixel(self, x, y, color):
        ...
        self.write_dma(None, _encode_pixel(color))
